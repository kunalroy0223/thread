  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        ...
        if (userData?.user) {
          const userInfo = {
            id: userData.user.uid, // <--- must be "id" so Profile.jsx sees it
            uid: userData.user.uid,
            email: userData.user.email,
            username: userData.user.displayName || formData.email
          };
          localStorage.setItem("user", JSON.stringify(userInfo));
          localStorage.setItem("uid", userData.user.uid); // optional, for auth context
          login(userInfo); // <--- now sets the proper id/email
        }
    ...
    try {
      ...
      if (userData?.user) {
        const userInfo = {
          ...
          uid: userData.user.uid,
          email: userData.user.email,
          username: userData.user.displayName || "Google User"
        };
        localStorage.setItem("user", JSON.stringify(userInfo));
    ...
    try {
      ...
      if (userData?.user) {
        const userInfo = {
          ...
          uid: userData.user.uid,
          email: userData.user.email,
          username: userData.user.displayName || "Apple User"
        };
        localStorage.setItem("user", JSON.stringify(userInfo));
        localStorage.setItem("uid", userData.user.uid);
        login(userInfo);
